...
fn parse_connection_string(conn_str: &str) -> Result<AcsConfig> {
    ...
    let endpoint = map
        .get("endpoint")
        .context("Connection string is missing 'endpoint'")?
        .to_string();
    let access_key = map
        .get("accesskey")
        .context("Connection string is missing 'accesskey'")?
        .to_string();
    Ok(AcsConfig {
        endpoint,
        access_key,
...
// --- Minimal SMTP Server Implementation ---
...
// Accept a WriteHalf for writing responses
...
async fn handle_connection(stream: TcpStream, mailer: Arc<dyn Mailer>) {
    ...
    loop {
        ...
        match reader.read_line(&mut line).await {
            ...
            Ok(_) => {
                ...
                info!(client_command = %cmd, "Received command");
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    info!("Initializing HTTP client and mailer...");
    let http_client = reqwest::Client::new();
    let mailer: Arc<dyn Mailer> = Arc::new(AcsMailer::new(http_client, acs_config.endpoint, acs_config.access_key, sender_address));
    let listener = TcpListener::bind(&listen_addr).await?;
    info!(listen_addr = %listen_addr, "Minimal SMTP relay listening for connections");
    loop {
        tokio::select! {
